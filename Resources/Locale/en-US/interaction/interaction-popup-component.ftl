...
petting-success-cargo-cyborg = You pet {THE($target)} on {POSS-ADJ($target)} hardworking metal head. #STARLIGHT
...
petting-failure-cargo-cyborg = You reach out to pet {THE($target)}, but {SUBJECT($target)} {CONJUGATE-BE($target)} busy hauling cargo! #STARLIGHT
